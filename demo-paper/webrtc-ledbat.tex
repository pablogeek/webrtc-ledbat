...
\begin{abstract} In this paper, we present what is, to the of our knowledge, the first
  implementation of LEDBAT for the WebRTC framework. In particular, we detail how we
  integrated the protocol in the WebRTC stack in such a way that all features provided by
  the existing transfer protocol (SCTP), as for instance
  in- and out-of-order delivery and reliable or unreliable delivery,
  could be maintained and applied to LEDBAT transfers. We also include preliminary results
  of this effort that show the
